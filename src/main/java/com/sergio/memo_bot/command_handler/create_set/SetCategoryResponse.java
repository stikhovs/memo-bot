...
public class SetCategoryResponse implements CommandHandler {
    ...
    public Reply getReply(ProcessableMessage processableMessage) {
        ...
        CategoryDto chosenCategory = categories.stream().filter(it -> it.getId().equals(categoryId)).findFirst().get();

        return BotMessageReply.builder()
                .chatId(chatId)
                .text("Набор будет создан под категорией \"%s\"".formatted(chosenCategory.getId()))
